...
jest.mock('../shared/validators', () => ({
  validateCreateOrderData: jest.fn(),
  sanitizeCreateOrderData: jest.fn(),
...
describe('initialize-order handler', () => {
  ...
  beforeEach(() => {
    ...
    process.env.INITIALIZE_ORDER_TOPIC_ARN = 'arn:aws:sns:us-east-1:123456789012:test-topic';
  });
  ...
  describe('successful order creation', () => {
    beforeEach(() => {
      (validateCreateOrderData as jest.Mock).mockReturnValue({ isValid: true });
      (sanitizeCreateOrderData as jest.Mock).mockReturnValue({
        cpf: '12345678901',
        email: 'test@example.com',
        name: 'João Silva',
        items: [{ id: 'item1', quantity: 2 }]
    ...
    test('should create order successfully', async () => {
      ...
      expect(validateCreateOrderData).toHaveBeenCalledWith({
        cpf: '12345678901',
        email: 'test@example.com',
        name: 'João Silva',
        items: [{ id: 'item1', quantity: 2 }]
      ...
      expect(sanitizeCreateOrderData).toHaveBeenCalled();
      expect(mockPublish).toHaveBeenCalled();
      expect(createSuccessResponse).toHaveBeenCalledWith(202, expect.objectContaining({
    ...
    test('should track performance', async () => {
      ...
      expect(PerformanceTracker).toHaveBeenCalledWith(mockLogger, 'initialize-order-request');
    ...
    test('should publish to SNS with correct parameters', async () => {
      ...
      expect(mockPublish).toHaveBeenCalledWith({
        ...
        Message: expect.stringContaining('"status":"requested"'),
  ...
  describe('validation failures', () => {
    test('should handle validation errors', async () => {
      (validateCreateOrderData as jest.Mock).mockReturnValue({
        isValid: false,
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('Validation failed', {
        error: 'Invalid CPF format',
        receivedFields: ['cpf', 'email', 'name', 'items']
      });
      ...
      expect(createErrorResponse).toHaveBeenCalledWith(400, 'Invalid CPF format');
