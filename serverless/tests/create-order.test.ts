...
describe('create-order handler', () => {
  ...
  beforeEach(() => {
    ...
    (generateCorrelationId as jest.Mock).mockReturnValue('order-test-correlation-id');
    ...
    (maskSensitiveData.email as jest.Mock).mockReturnValue('test***@example.com');
    (maskSensitiveData.name as jest.Mock).mockReturnValue('Jo***');

    mockSend.mockImplementation((command: any) => {
      if (command.constructor.name === 'QueryCommand' || command.input?.IndexName === 'email-index') {
        return Promise.resolve({ Items: [] });
      }
      
      if (command.constructor.name === 'GetCommand' || command.input?.Key?.productId) {
      ...
      if (command.constructor.name === 'UpdateCommand') {
        const productId = command.input?.Key?.productId;
        const quantityToSubtract = command.input?.ExpressionAttributeValues?.[':quantity'];
      ...
      Records: [
        {
          ...
          eventSource: 'aws:sqs',
          eventSourceARN: 'arn:aws:sqs:us-east-1:123456789012:create-order-queue',
          awsRegion: 'us-east-1'
        } as SQSRecord
      ]
    };
  ...
  describe('Successful Processing', () => {
    test('should process single order successfully', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Processing SQS batch started', {
        recordsCount: 1
      });

      expect(mockLogger.info).toHaveBeenCalledWith('SQS batch processing completed', {
        totalRecords: 1
      });

      expect(PerformanceTracker).toHaveBeenCalledWith(mockLogger, 'create-order-batch');
    ...
    test('should process order data and mask sensitive information', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Order data parsed successfully', {
        customerCpf: '123***901',
        customerEmail: 'test***@example.com',
        customerName: 'Jo***',
        itemsCount: 2,
        orderStatus: 'requested',
        orderTimestamp: '2025-08-03T10:30:00.000Z'
      });
    });
    ...
    test('should create order with correct status and calculate total items', async () => {
      ...
      const orderCreatedCall = mockLogger.info.mock.calls.find((call: any) => 
        call[0] === 'Order created successfully'
      );
    ...
    test('should search for existing lead and create order', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Created new lead for order', {
    ...
    test('should process multiple records in batch', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Order created successfully', {
        newStatus: 'PENDING',
        totalItems: 5,
        totalValue: 225.00,
        leadId: expect.stringMatching(/^lead-\d+-[a-z0-9]+$/),
    ...
    test('should enrich order items with product information and calculate total value', async () => {
      ...
      const itemEnrichedCalls = mockLogger.info.mock.calls.filter((call: any) => 
        call[0] === 'Item enriched successfully'
      );
      ...
      const enrichmentCompletedCall = mockLogger.info.mock.calls.find((call: any) => 
        call[0] === 'Order items enrichment completed'
      );
  ...
  describe('Error Handling', () => {
    test('should handle invalid JSON in SNS message', async () => {
      ...
      expect(mockTracker.finishWithError).toHaveBeenCalledWith(expect.any(Error));
    });
  ...
  describe('Logging Context', () => {
    ...
    test('should log record processing details', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Starting order creation logic');
    });
  });
  ...
  describe('Stock Management', () => {
    describe('Products with Stock Control', () => {
      test('should successfully update stock for products with quantityInStock', async () => {
        ...
        const updateCalls = mockSend.mock.calls.filter((call: any) => 
          call[0].constructor.name === 'UpdateCommand'
        );
        expect(updateCalls).toHaveLength(1);
        
        const updateCommand = updateCalls[0][0];
        expect(updateCommand.input.Key.productId).toBe('item1');
        expect(updateCommand.input.UpdateExpression).toContain('SET quantityInStock = quantityInStock - :quantity');
        expect(updateCommand.input.ExpressionAttributeValues[':quantity']).toBe(5);
        expect(updateCommand.input.ConditionExpression).toBe('quantityInStock >= :quantity AND isActive = :isActive');
      });
    ...
    describe('Products without Stock Control', () => {
      test('should process products without quantityInStock successfully', async () => {
        ...
        const orderPutCalls = mockSend.mock.calls.filter((call: any) => 
          call[0].constructor.name === 'PutCommand' && 
          call[0].input?.Item?.orderId
        );
